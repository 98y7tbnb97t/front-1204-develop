...
const EstimateAutoSMS: FC = () => {
  const dispatch = useAppDispatch();
  const { estimateAutoSMS } = useAppSelector((state) => state.InfoTextsSlice);
  ...
  useEffect(() => {
    if (estimateAutoSMS) {
      setText(estimateAutoSMS);
    }
  }, [estimateAutoSMS]);

  const handleSave = () => {
    void dispatch(editInfoText({ field: EInfoTextFields.estimateAutoSMS, text }))
    setIsModalOpen(true);
  };
...
export default EstimateAutoSMS;