...
import { getAdvicesForTraners, saveAdvicesForTraners } from '../../store/reducers/InfoTextsSlice';
...
const AdvicesForTranersTexts: FC = () => {
    const dispatch = useAppDispatch();
    const advicesForTraners = useAppSelector(state => state.InfoTextsSlice[EInfoTextFields.advicesForTraners]);
    ...
    const [redTextLanguage, setRedTextLanguage] = useState<Elanguages>(Elanguages.RU);
    const languageGlobal = useAppSelector(state => state.TranslateSlice.language);
    ...
    useEffect(() => {
        if (advicesForTraners) {
            setRedTextAbove(advicesForTraners.redTextAbove);
            setTextBlocks(advicesForTraners.textBlocks.map(block => ({
                ...block,
    ...
    const saveAllData = async () => {
        try {
            await dispatch(saveAdvicesForTraners({ redTextAbove, textBlocks, videoFiles, videosToDelete })).unwrap();
            setIsModalOpen(true);
            setVideoFiles([]);
            setVideosToDelete([]);
        } catch (error) {
    ...
    return (
        <>
            <div className="mt-5 flex flex-col gap-2">
                <label className='font-bold text-xl'>{translations.advicesForTraners.redTextAbove[languageGlobal]}</label>
                <div className="flex">
                    <Input
                        type="text"
                        wrapperClasses='grow'
                    ...
                    />
                    <Button onClick={saveAllData} className="w-max ml-2 bg-green-500 hover:bg-green-600">{translations.advicesForTraners.saveChanges[languageGlobal]}</Button>
                </div>
                <ChangeLanguageBtns className="mt-2" language={redTextLanguage} onLangChange={setRedTextLanguage}/>
            </div>
            <ReactSortable 
            ...
            >
                {textBlocks.map((block) => (
                    <div key={block.id} className="mt-5 flex flex-col gap-2 bg-gray-100 p-4 rounded-lg relative group">
                        ...
                        <div className="ml-8">
                            <label className='font-bold text-xl'>{translations.advicesForTraners.title[languageGlobal]}</label>
                            <Input
                                type="text"
                                value={block.title[block.language] || ''}
                                onChange={(e) => updateTextBlock(block.id, 'title', e.target.value)}
                            />
                            <label className='font-bold text-xl mb-2'>{translations.advicesForTraners.content[languageGlobal]}</label>
                            <div className="mt-2 not-draggable">
                            ...
                            <label className='font-bold text-xl mt-2'>{translations.advicesForTraners.videos[languageGlobal]}</label>
                            <div className="flex items-center flex-wrap">
                                ...
                                <Button onClick={saveAllData} className="w-max ml-2 mt-2 bg-green-500 hover:bg-green-600">{translations.advicesForTraners.saveChanges[languageGlobal]}</Button>
                            </div>
                            <div className='mt-4'>
            ...
            <Button onClick={addTextBlock} className="mt-5">{translations.advicesForTraners.addBlock[languageGlobal]}</Button>

            <Modal active={isModalOpen} setActive={setIsModalOpen}>
                <div className="p-4 text-center">
                    <h2 className="text-xl font-bold mb-2">{translations.advicesForTraners.successMessage[languageGlobal]}</h2>
                    <p>{translations.advicesForTraners.dataSaved[languageGlobal]}</p>
                    <Button onClick={() => setIsModalOpen(false)} className="mt-4">{translations.advicesForTraners.close[languageGlobal]}</Button>
                </div>
            </Modal>
        </>
    );
...
export default AdvicesForTranersTexts;