...
const StudentAddedToGroupText: FC = () => {
  const dispatch = useAppDispatch();
  const { studentAddedToGroupText } = useAppSelector((state) => state.InfoTextsSlice);
  ...
  useEffect(() => {
      void dispatch(getInfoText({ field: EInfoTextFields.studentAddedToGroup }));
  }, [dispatch]);

  useEffect(() => {
    if (studentAddedToGroupText) {
      setText(studentAddedToGroupText);
    }
  }, [studentAddedToGroupText]);

  const handleSave = () => {
    void dispatch(editInfoText({ field: EInfoTextFields.studentAddedToGroup, text }))
    setIsModalOpen(true);
  };
...
export default StudentAddedToGroupText;