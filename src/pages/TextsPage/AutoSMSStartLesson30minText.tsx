...
const AutoSMSStartLesson30minText: FC = () => {
  const dispatch = useAppDispatch();
  const { AutoSMSStartLesson30min } = useAppSelector((state) => state.InfoTextsSlice);
  ...
  useEffect(() => {
    if (AutoSMSStartLesson30min) {
      setText(AutoSMSStartLesson30min);
    }
  }, [AutoSMSStartLesson30min]);

  const handleSave = () => {
    void dispatch(editInfoText({ field: EInfoTextFields.AutoSMSStartLesson30min, text }))
    setIsModalOpen(true);
  };
...
export default AutoSMSStartLesson30minText;