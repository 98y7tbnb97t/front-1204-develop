...
const HomeworkReminder: FC = () => {
  const dispatch = useAppDispatch();
  const { homeworkReminder } = useAppSelector((state) => state.InfoTextsSlice);
  ...
  useEffect(() => {
      void dispatch(getInfoText({ field: EInfoTextFields.homeworkReminder }));
  }, [dispatch]);

  useEffect(() => {
    if (homeworkReminder) {
      setText(homeworkReminder);
    }
  }, [homeworkReminder]);

  const handleSave = () => {
    void dispatch(editInfoText({ field: EInfoTextFields.homeworkReminder, text }))
    setIsModalOpen(true);
  };
