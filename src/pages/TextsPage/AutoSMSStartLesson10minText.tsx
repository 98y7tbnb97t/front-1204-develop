...
const AutoSMSStartLesson10minText: FC = () => {
  const dispatch = useAppDispatch();
  const { AutoSMSStartLesson10min } = useAppSelector((state) => state.InfoTextsSlice);
  ...
  useEffect(() => {
      void dispatch(getInfoText({ field: EInfoTextFields.AutoSMSStartLesson10min }));
  }, [dispatch]);

  useEffect(() => {
    if (AutoSMSStartLesson10min) {
      setText(AutoSMSStartLesson10min);
    }
  }, [AutoSMSStartLesson10min]);

  const handleSave = () => {
    void dispatch(editInfoText({ field: EInfoTextFields.AutoSMSStartLesson10min, text }))
    setIsModalOpen(true);
  };
...
export default AutoSMSStartLesson10minText;