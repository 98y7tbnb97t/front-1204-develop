...
const HomeworkDeadlineAdvice: FC = () => {
  const dispatch = useAppDispatch();
  const { homeworkDeadlineAdvice } = useAppSelector((state) => state.InfoTextsSlice);
  ...
  useEffect(() => {
      void dispatch(getInfoText({ field: EInfoTextFields.homeworkDeadlineAdvice }));
  }, [dispatch]);

  useEffect(() => {
    if (homeworkDeadlineAdvice) {
      setText(homeworkDeadlineAdvice);
    }
  }, [homeworkDeadlineAdvice]);

  const handleSave = () => {
    void dispatch(editInfoText({ field: EInfoTextFields.homeworkDeadlineAdvice, text }))
    setIsModalOpen(true);
  };
...
export default HomeworkDeadlineAdvice;