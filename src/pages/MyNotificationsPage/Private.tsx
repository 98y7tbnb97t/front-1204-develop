...
const Private:FC = () => {
  ...
  useEffect(() => {
    const fetchData = () => {
      void dispatch(getAllAutoSMS({ user_id: user._id, isPrivate: true }))
    }
    if (user._id) {
      void fetchData();
    }
  ...
  return (
    <div className="w-full p-5 flex flex-col gap-5">
        <h2 className="text-center text-xl font-medium">{myNotifications[language]}</h2>
        <div className="flex gap-2 justify-end">
          <Button onClick={() => setAddAutoSMSModal(true)} className="bg-sky-500 max-w-[400px]">{addNotificationsForYourself[language]}</Button>
          <Button onClick={() => setDetailsModal(true)} className="max-w-[200px]">{moreDetails[language]}</Button>
        </div>
				{autoSMSList.map(autoSMS => (
          <div key={autoSMS._id} className={classNames("flex justify-between items-center mb-2 border-b border-gray-300  p-2", {
