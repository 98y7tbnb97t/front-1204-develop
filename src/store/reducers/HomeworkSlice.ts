...
export const getHomeworks = createAsyncThunk<IHomework[], {group_id?: string, page?: number}>(
    'homeworkSlice/getHomeworks',
    async (data, {rejectWithValue}) => {
        try {
            const { group_id, page } = data;
            const response = await HomeworkService.getHomeworks(group_id, page);
