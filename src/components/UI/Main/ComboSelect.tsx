...
const ComboSelect: FC<ComboSelectProps> = ({
                                               options,
                                               value,
                                               onChange,
                                               multiple,
    ...
    return (
        <div className={['relative w-full', wrapperClass].join(' ')}>
            <Combobox
                multiple={multiple}
                id='selectFilter'
                className={['mb-5 text-gray-700 placeholder-gray-400  rounded-md focus:border-apricot focus:ring-apricot focus:outline-none focus:ring focus:ring-opacity-40', className].join(' ')}
                as="div"
                value={value}
                onChange={(e: never) => {
                    selectHandler(e);
            ...
            >
                <Combobox.Input
                    placeholder={value.length ? value.map(item => item.name).join() : name}
                    onInput={(event) => setQuery(event.target.value)} className={`border border-[#B7975A] bg-white rounded-full py-2 px-2 md:px-5 w-full placeholder-black placeholder:font-medium focus:placeholder-[#ccc]`}/>
