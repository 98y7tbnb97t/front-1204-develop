...
const ChessBoard: FC<ChessBoardProps> = ({ position, materialId, movesM, setMoves, PrevBackTheme, game, setGame, type}) => {
    ...
    async function onDrop(sourceSquare, targetSquare, piece) {
        ...
        if(move) {
            ...
            const moves = homeworkSl.game[0] ? homeworkSl.game[0].moves : [];
            if(groupId) {
                await HomeworkService.editHomework(groupId, {material: materialId, move: {user_id: user._id, name: user.name || testUser.name, sname: user.sname || testUser.sname, moves: [...moves, {color: move.color, move: move.san}]}, movesHistory: game.history()});
                dispatch(setMovesState({user_id: user._id, name: user.name || testUser.name, sname: user.sname || testUser.sname, color: move.color, move: move.san}));
            }
    ...
    function getPositionObject(currentPosition) {
        if(Object.values(currentPosition).find((item) => item === "bP")) {
    ...
    return (
        <div className={` max-w-full ${!isDeviceMobile() ? "board-desk" : "board-mob"} overflow-hidden`}>
            <div id="board_wrapper">
                ...
                <Chessboard
                    ...
                    getPositionObject={type === 'customtask' ? getPositionObject : ()=>{}}
                    ...
                    customPieces={type === 'customtask' && {
                        "bP": (props) => <img style={{margin: "10% auto"}} width='80%' height='80%' src={Star}/>,
                    }}
                />
            </div>
            <div className="flex items-center justify-around  border-2 border-[#ccc] rounded-full py-2 mt-2 playbar">
                <button title='Глава назад' className=' text-base text-red-500' onClick={() => PrevBackTheme(false)}><FaFastBackward/></button>
                <button title='Все ходы назад' className=' text-xl' onClick={() => fastBack()}><AiFillFastBackward/></button>
                <button title='Ход назад' className=' text-xl' onClick={() => back()}><AiFillStepBackward/></button>
                <button title='Ход вперед' className=' text-xl' onClick={() => next()}><AiFillStepForward/></button>
                <button title='Все ходы вперед' className=' text-xl' onClick={() => fastNext()}><AiFillFastForward/></button>
                <button title='Глава вперед' className=' text-base text-red-500' onClick={() => PrevBackTheme(true)}><FaFastForward/></button>
                <button title='В исходную позицию' className=' text-xl' onClick={() => safeGameMutate((game) => { game.load(position);})}><AiOutlineCloseCircle/></button>
