...
const UploadFile: FC<UploadFileProps> = ({setFilesHandler, className, labelClass, theme, multiple, id, accept}) => {
    ...
    return (
        <div className={className ? className : ''}>
            ...
            <label htmlFor={id} className={['hover:bg-gray-700 py-1 xl:py-2 cursor-pointer flex px-1 xl:px-3 rounded-sm transition-all mr-2 text-white text-xl', theme === 'dark' && '!text-gray-900 hover:!text-white', labelClass].join(' ')}><ImAttachment/></label>
        </div>
    )
}
