...
const AddUserToGroupModal2: FC<AddUserToGroupModal2ModalProps> = ({ modal, setModal, email, user_id }) => {
    ...
    const {
        ...
        enterGroupNameText: ITranslateItemString,
        suggestText: ITranslateItemString,
    ...
    return (
        <>
            <Modal active={modal} setActive={setModal} className='items-center border-2 border-[#8A6E3E] !rounded-3xl !max-w-[800px] !w-full'>
                ...
                <p className='text-center pt-5'>{enterGroupNameText[language]}</p>
                <div className="text-left">
                    <p className='mb-2 text-lg font-bold mt-5'>{suggestText[language]}</p>
                    <div>
                        {recs.length > 0 &&
                            recs.map(item=>
                ...
                <form onSubmit={(e) => void handleSubmit(onSubmit)(e)} className='bg-white container mx-auto flex flex-col px-10 py-5 max-w-2xl'>
                    <Input wrapperClass='mb-5' type="text" label={groupNameText[language]} placeholder={groupNameText[language]} error={errors.group_id?.message} register={register('group_id', { required: "The field must be filled" })} onInput={e=> void testdebounce(e.target.value)}/>
