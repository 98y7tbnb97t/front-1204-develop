...
const MoveUserToGroupModal: FC<MoveUserToGroupModalProps> = ({ modal, setModal, user }) => {
    ...
    const onSubmit: SubmitHandler<Form> = async (data, e) => {
        e?.preventDefault();
        if(userid) {
            await DialogService.addUserToChat(user.email, data.group_id).then(()=> setModal(false)).catch((e: AxiosError)=> {
                const event = e.response?.data as ServerError;
                setModalError(event.error)
                setEModal(true);
            });
            await DialogService.removeUserFromChat(user._id, userid).then(()=> setModal(false)).catch((e: AxiosError)=> {
                const event = e.response?.data as ServerError;
                setModalError(event.error)
                setEModal(true);
            });
    ...
    return (
        <>
            <Modal active={modal} setActive={setModal} className='items-center'>
                ...
                <p className='text-center pt-5'>{enterTheIdOfGroupText[language][0]} {user.email} {enterTheIdOfGroupText[language][1]}</p>
