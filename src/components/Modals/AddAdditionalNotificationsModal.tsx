...
import { createAutoSMS } from "../../store/reducers/AutoSMSSlice";
...
const AddAdditionalNotificationsModal:FC<AutoSMSModalProps> = ({ className, active, setActive, autoSMS }) => {
  ...
  const { addText, addNotification, sendReminderTo, textT, nameT, save } = translations.autoSMS;
  ...
  const saveHandler = () => {
    ...
    void dispatch(createAutoSMS({ 
      ...
      group_id: autoSMS.group_id,
      repeat: repeatOption,
      notifications: notificationGroups,
      text, 
      sendToNotifications: sendToNotifications,
      title,
      enabled: true,
      usersToSend: [user._id],
      additionalContent: autoSMS.additionalContent,
      isPersonal: true,
      isAdditional: true
    }));
    setActive(false);
    ...
    <Modal maxWidth={800} active={active} setActive={setActive} className={classNames('p-4 !bg-white', {}, [className])}>
      ...
      { !autoSMS.group_id && (
        ...
        <textarea
          ...
          placeholder={textT[language]}
        />
        <ChangeLanguageBtns className="mt-2" language={language} onLangChange={setLanguage}/>
        {!autoSMS.group_id && <div className="flex items-center gap-2 mt-2">
...
export default AddAdditionalNotificationsModal;