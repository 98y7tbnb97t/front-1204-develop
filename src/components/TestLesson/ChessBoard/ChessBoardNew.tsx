...
const ChessBoard: FC<ChessBoardProps> = ({moveMode, globalMode, clear, setClear, position, materialId, movesM, setMoves, PrevBackTheme, type, custom, game, setGame}) => {
    ...
    useEffect(() => {
        ...
        socket.on("group:recive_remove_arrow", (data) => {
            ref.current.removeExactArrow(data.fromx, data.fromy, data.tox, data.toy);
        });
    }, [socket])
    ...
    return (
        <div className='w-[85vmin]'>
            <div id="board_wrapper">
                <canvas id="primary_canvas" className='w-[85vmin] aspect-square'></canvas>
                <canvas id="drawing_canvas" className='w-[85vmin] aspect-square'></canvas>
                <Chessboard
