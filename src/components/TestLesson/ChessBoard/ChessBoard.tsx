...
const ChessBoard: FC<ChessBoardProps> = ({moveMode, globalMode, clear, setClear, position, materialId, movesM, setMoves, PrevBackTheme, type, custom, game, setGame}) => {
    ...
    useEffect(() => {
        ...
        socket.on("group:recive_remove_arrow", (data) => {
            ref.current.removeExactArrow(data.fromx, data.fromy, data.tox, data.toy, true);
        });
    }, [socket])
            ...
            // if promotion move
                ...
                if(move) {
                    ...
                    if(groupId) {
                        ...
                        GroupMakeMoveSocket({room: groupId, user_id: user._id, color: move.color, move: move.san, role: user.role});
                    }
    ...
    const CustomPieceRenderer = (props) => {
        ...
        return (
            <>
                {(tmp.includes(square))
                ?
                    <img style={{margin: "10% auto"}} width='80%' height='80%' src={Star} alt="" />
                :
    ...
    return (
        <div className='w-full xl:w-[85vmin]'>
            <div id="board_wrapper">
                <canvas id="primary_canvas" className='!w-full sm:w-[calc(100vh-281px)] h-[calc(100vh-281px)] max-2xl:w-[calc(100vh-200px)] max-2xl:h-[calc(100vh-200px)]'></canvas>
                <canvas id="drawing_canvas" className='!w-full sm:w-[calc(100vh-281px)] h-[calc(100vh-281px)] max-2xl:w-[calc(100vh-200px)] max-2xl:h-[calc(100vh-200px)]'></canvas>
                <Chessboard
            ...
            <div className="flex items-center justify-center border-2 border-[#ccc] rounded-full py-2 mt-2 max-2xl:py-1">
                {(user.role === 'DIRECTOR' || user.role === 'ZDIRECTOR' || user.role === 'TRANER' || user.role === 'TRANERMETODIST') &&
                    <button title='Глава назад' className='mr-4 text-base text-red-500 max-2xl:text-sm' onClick={() => PrevBackTheme(false)}><FaFastBackward/></button>
                }
                <button className='mr-4 text-xl max-2xl:text-base' title='Все ходы назад' onClick={() => fastBack()}><AiFillFastBackward/></button>
                <button className='mr-4 text-xl max-2xl:text-base' title='Ход назад' onClick={() => back()}><AiFillStepBackward/></button>
                <button className='mr-4 text-xl max-2xl:text-base' title='Ход вперед' onClick={() => next()}><AiFillStepForward/></button>
                <button className='mr-4 text-xl max-2xl:text-base' title='Все ходы вперед' onClick={() => fastNext()}><AiFillFastForward/></button>
                {(user.role === 'DIRECTOR' || user.role === 'ZDIRECTOR' || user.role === 'TRANER' || user.role === 'TRANERMETODIST') &&
                    <button title='Глава вперед' className='mr-4 text-base max-2xl:text-sm text-red-500' onClick={() => PrevBackTheme(true)}><FaFastForward/></button>
                }
                <button className='mr-4 text-xl max-2xl:text-base' title='В исходную позицию' onClick={() => safeGameMutate((game) => { game.load(position);})}><AiOutlineCloseCircle/></button>
